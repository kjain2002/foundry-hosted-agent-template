...
Standing up a hosted agent is **two separate steps**. Infrastructure-as-code (Terraform/Bicep) only covers the first; `azd deploy` always does the second.

| Phase | What it does | Done by |
|---|---|---|
| **1. Provision** | Create the Azure resources — Foundry project, model deployment, App Insights, RBAC | `azd provision` **— or** Terraform (this `terraform` branch) / Bicep (`main`) |
| **2. Deploy** | Build the agent code into a container and ship it to the Foundry hosted runtime | `azd deploy` (IaC can't do this) |

> Terraform/Bicep are an **alternative to `azd provision`** — the "build the house" step. `azd deploy` is the "move the furniture in" step and is **always** required.
...
Use **Terraform** (this `terraform` branch → `infra/terraform/`) or **Bicep** (the `main` branch → `infra/bicep/`) for Phase 1, then `azd deploy` for Phase 2. See [Provisioning](#provisioning-infrastructure) below.
...
**This branch ships Terraform.** Stand up the Foundry project with:

- [`infra/terraform/`](infra/terraform/) — provisions a project + model deployment + optional App Insights + RBAC on an existing Foundry account (`terraform init && terraform apply -var-file=example.tfvars`).
- Prefer Bicep? The **`main` branch** carries the same template with `infra/bicep/` instead.
- Already have a project? Skip IaC and just set `FOUNDRY_PROJECT_ENDPOINT`.
