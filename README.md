...
> [!TIP]
> This repo's [`infra/bicep/`](infra/bicep/README.md) adds a **project + model** to an **existing Foundry account**. It does **not** create the account or the Standard storage/Cosmos — use the official templates below for those.
...
**Step 1 — Copy the example params:**

```powershell
Copy-Item infra/bicep/example.bicepparam infra/bicep/my.bicepparam
```

**Step 2 — Fill in `my.bicepparam`** (at least these):
...
**Step 3 — Deploy at the account's resource group:**

```powershell
az deployment group create `
  --resource-group <your-resource-group> `
  --template-file infra/bicep/main.bicep `
  --parameters infra/bicep/my.bicepparam
```
...
> Prefer **Terraform**? Same thing lives on the **`terraform` branch** → `infra/terraform/`.
...
├── infra/
│   └── bicep/
│       ├── main.bicep          # Adds project + model (+ App Insights + RBAC) to an account
│       ├── example.bicepparam  # Copy to your own .bicepparam and fill in values
│       └── README.md           # How to deploy the Bicep
